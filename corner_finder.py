...
def file_name(original_name, suffix):
    return '{name}-{suffix}.jpeg'.format(name="".join(original_name.split(".")[:-1]), suffix=suffix)
...
def corners2(prep_image):
    ...
    return [closest_node((0, 0), points), closest_node((width, 0), points), closest_node((0, height), points), closest_node((width, height), points)]
...
def main():
    ...
    for file in glob.glob(pattern):
        ...
        base_corners = list(
            chain.from_iterable(zip(coordinates, [(0, 0), (width, 0), (0, height), (width, height)]))
        )
